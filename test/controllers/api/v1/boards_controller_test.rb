...
class Api::V1::BoardsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create board" do
    assert_difference("Board.count", 1) do
      post api_v1_boards_path, params: { board: { name: "New Board", description: "Test" } }
    end
  ...
  test "update board as creator" do
    board = create(:board, creator: @user)
    patch api_v1_board_path(board), params: { board: { name: "Updated" } }
    assert_response :ok
  ...
  test "update board as non-creator is forbidden" do
    ...
    patch api_v1_board_path(board), params: { board: { name: "Hacked" } }
    assert_response :forbidden
  end
  ...
  test "destroy board as creator" do
    board = create(:board, creator: @user)
    assert_difference("Board.count", -1) do
      delete api_v1_board_path(board)
    end
